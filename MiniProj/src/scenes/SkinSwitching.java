...
public class SkinSwitching {
	...
	public SkinSwitching(SceneManager sceneManager) {
		...
		next1.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				if(player1Selection < 2)player1Selection++;
				else player1Selection = 0;
		...
		prev1.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				if(player1Selection > 0)player1Selection--;
				else player1Selection = 2;
				showSelections(p1, true, player1Selection);
			}
		});
		...
		next2.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				if(player2Selection < 2)player2Selection++;
				else player2Selection = 0;
		...
		prev2.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				if(player2Selection > 0)player2Selection--;
				else player2Selection = 2;
				showSelections(p2, false, player2Selection);
			}
		});
    ...
    private void assignStyleSheets() {
        skinSwitchingScene.getStylesheets()
    		.add((getClass())
    		.getResource("skin.css")
    		.toExternalForm());
	...
	private void showSelections(GraphicsContext draw, boolean isPlayer1, int selection) {
		...
		case 0:
			if(isPlayer1) selectedImageP1 = Graphics.SKIN_1;
			else selectedImageP2 = Graphics.SKIN_1;
			break;
		// case where player selects second skin (red jeep)	
		case 1:
			if(isPlayer1) selectedImageP1 = Graphics.SKIN_2;
			else selectedImageP2 = Graphics.SKIN_2;
			break;
		// case where player selects third skin (blue jeep)
		case 2:
			if(isPlayer1) selectedImageP1 = Graphics.SKIN_3;
			else selectedImageP2 = Graphics.SKIN_3;
			break;
		// catching case
		default:
			if(isPlayer1) selectedImageP1 = Graphics.SKIN_1;
		...
		if(isPlayer1) draw.drawImage(selectedImageP1,345,227,104,66);
		else draw.drawImage(selectedImageP2,345,470,104,66);
	}
